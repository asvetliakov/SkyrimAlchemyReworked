...
namespace {
    class KIDEventHandler : public RE::BSTEventSink<SKSE::ModCallbackEvent> {
    public:
        static KIDEventHandler* GetSingleton() {
            static KIDEventHandler singleton;
            return &singleton;
        }
    ...
    protected:
        ...
        EventResult ProcessEvent(const SKSE::ModCallbackEvent* a_event,
                                 RE::BSTEventSource<SKSE::ModCallbackEvent>*) override {
            if (a_event && a_event->eventName == "KID_KeywordDistributionDone") {
                logger::info("KID completed, initializing");

                AlchmeyDistributor::Initialize();
    ...
    private:
        KIDEventHandler() = default;
        KIDEventHandler(const KIDEventHandler&) = delete;
        KIDEventHandler(KIDEventHandler&&) = delete;

        ~KIDEventHandler() override = default;

        KIDEventHandler& operator=(const KIDEventHandler&) = delete;
        KIDEventHandler& operator=(KIDEventHandler&&) = delete;
    };
    ...
    void InitializeMessaging() {
        // GetModCallbackEventSource()->AddEventSink([](ModCallbackEvent * event) {
        //     event.
        // })
        GetModCallbackEventSource()->AddEventSink(KIDEventHandler::GetSingleton());
        ...
        if (!GetMessagingInterface()->RegisterListener([](MessagingInterface::Message* message) {
                switch (message->type) {
                    ...
                    case MessagingInterface::kDataLoaded:  // All ESM/ESL/ESP plugins have loaded, main menu is now
                        ...
                        // AlchmeyDistributor::InstallHooks();
                        break;
